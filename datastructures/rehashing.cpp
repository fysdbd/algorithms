...
/*
    ...
    description(lintcode:
                https://www.lintcode.com/en/problem/rehashing/
                )
...
public:
    ...
    vector<ListNode*> rehashing(vector<ListNode*> hashTable) {
        vector<ListNode*> result(hashTable.size()*2, nullptr);
        for (auto item : hashTable) {
            while (item != nullptr) {
                int val = item->val;
                item = item->next;
                int code = hashcode(val, result.size());

                auto entry = result[code];
                if (!entry) { result[code] = new ListNode(val); continue; }
                while (entry->next != nullptr) { entry = entry->next; }
                entry->next = new ListNode(val);
            }
        }
